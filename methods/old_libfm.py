...
class FM(object):
    ...
    def run(self):
        predictions = libfm_predict(
            self.ftrain, self.ftest, self.outfile, self.iter, self.std,
            self.dim, self.bias, self.task, self.fbias, self.gbias)
        return np.round(predictions) if self.task == 'c' else predictions


def fm_mcmc(method='mcmc', task='r', iter=100, std=0.1, dim=8,
...
def libfm_model2(train, test, *args, **kwargs):
    ...
    to_predict[target] = fm_mcmc(train, test, *args, **kwargs)
    return to_predict
...
if __name__ == "__main__":
    ...
    def eval_fm(train, test):
        return libfm_model2(
            ...
            gbias=args.gbias, task=args.task, target=target,
            cvals=cvals, rvals=rvals, pcgrades=args.pcgrades)
